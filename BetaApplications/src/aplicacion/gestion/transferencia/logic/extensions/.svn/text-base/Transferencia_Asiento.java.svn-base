...
import java.util.*;
public class Transferencia_Asiento  extends Logic_Extension {
	
	public List<String> getInstruccionesDeAsiento(){
		...
		if (medios_de_pago.size()>0){
			debe_haber="H";
			caja=frame.get_list_caja_origen().getSelectedItem().toString();
			
			int sec=0;
			String[] qs=new String[2*medios_de_pago.size()];
			...
			caja=frame.get_list_caja_destino().getSelectedItem().toString();
			debe_haber="D";
			...
			for (int i=0;i<medios_de_pago.size();i++){
				...
				if (cuenta2.compareTo("111010002")==0){
					...
					importe2.replace(",", "");
					if (impx>0){
						qs[medios_de_pago.size()+i]=this.getInsertACuentaCHT(mes, asiento, ""+sec,detalle, fecha, cuenta2, descripcion2, obs, cb_tc, cb_idcomp, debe_haber, importe2, caja, cht_idbanco, cht_serie, cht_numero, cht_vencimiento, importe2);						
					}
				...
				if (cuenta2.compareTo("42101")==0){
					if (impx>0){
						importe2.replace(",", "");
						qs[medios_de_pago.size()+i]=this.getInsertACuenta(mes, asiento, ""+sec, detalle,fecha, cuenta2, descripcion2, obs, cb_tc, cb_idcomp, debe_haber, importe2, caja);						
					}
				}
				if (cuenta2.compareTo("111010001")==0){
					importe2.replace(",", "");
	...
	public String getInsertIntoCbs(String tc,String idcomp,String cuenta,String fecha,String importe,String anticipo){
		String q="insert into b_cbs (tc,idcomprobante,cuenta,fecha,importe,anticipo,anulada) values (";
		q=q+"'"+tc+"',";
		q=q+"'"+idcomp+"',";
		q=q+"'"+cuenta+"',";
		q=q+"'"+fecha+"',";
