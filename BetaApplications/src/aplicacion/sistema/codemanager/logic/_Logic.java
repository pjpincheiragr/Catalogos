package aplicacion.sistema.codemanager.logic;
...
import aplicacion.sistema.codemanager.constructor.*;
...
import aplicacion.sistema.codemanager.logic.Nodo;
...
public class _Logic extends Logic {
	    	...
	    	if (parent!=null){
	    		...
	    		if (results!=null){
	    			if (results.length>0){
	    				...
	    				if (tecnica!=null){
	    					if (tecnica.length>0){
	    						String dato=(String) results[0][0];
	    						frame.get_txt_info().setText(dato);
	    					}
	...
	private boolean store_clasifications(){
		...
		dato=(String)frame.get_txt_info().getText();
...
public void _evaluate_dato(JTextArea tx,int row,int col){
