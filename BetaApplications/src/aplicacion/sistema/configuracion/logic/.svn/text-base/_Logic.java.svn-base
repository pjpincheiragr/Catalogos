...
public class _Logic extends Logic {
	...
	private void create_table(Object[][] results) {
		...
		aplicacion.sistema.configuracion.constructor._Constructor C=
			(aplicacion.sistema.configuracion.constructor._Constructor) this.getConstructor();
	...
	public void evaluar_tabla_value(JTextField tx,int row){
		...
		if (data.existe(id)){
				data.update(id, valor);
		}else{
				data.insert(id, valor);
		}
		DefaultTableModel model=(DefaultTableModel) frame.getJTable().getModel();
