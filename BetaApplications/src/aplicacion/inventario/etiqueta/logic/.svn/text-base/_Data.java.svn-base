...
public class _Data extends Data {
	...
	public Object[][] getStock(String idproveedor,String linea,String idarticulo_desde,String idarticulo_hasta,String iddeposito,boolean restriction){
		return this.getQueryResult(this.getStockQuery(idproveedor, linea, idarticulo_desde, idarticulo_hasta,iddeposito,restriction));
	}
	
	public String getStockQuery(String idproveedor,String linea,String idarticulo_desde,String idarticulo_hasta,String iddeposito,boolean restriction){
		String q="";
		q+="select ";
		...
		q+="group by a.idarticulo,a.descripcion,a.descripabrev ";
		if (restriction){
			q+="having ";
			if (iddeposito.compareTo("")!=0){
				q+="sum(case when s.iddeposito like '"+iddeposito+"' then s.cantidadud else 0 end)>0 ";
			}else{
				q+="sum(s.cantidadud)>0 ";
			}
	...
	public Object[][] getHistorial(String desde,String hasta,boolean todas){
		return this.getQueryResult(this.getHistorialQuery(desde, hasta, todas));
	}
	
	public String getUpdate(String id,double qty,boolean impresa){
		String q="";
		q+="update b_etiquetas set cantidad="+qty+",";
		if (impresa){
			q+="impresa=1 ";	
		}else{
			q+="impresa=0 ";
	...
	public String getHistorialQuery(String desde,String hasta,boolean todas){
		String q="";
		q+="select idarticulo,descripcion,cantidad,especial,impresa,id ";
		q+="from b_etiquetas";
		String where="";
	...
	public String getCpteStockFCN(String idarticulo,String idcomprobante,String idproveedor,String nombre){
		String q="";
		q=q+" select top 50 c.cuenta,m.descripcion,c.fecha_comprobante,c.tc,c.idcomprobante,c.fecha_carga "; 
		q=q+" from b_fcn c left outer join ";
		q=q+" ma_cuentas m on c.cuenta=m.codigo  ";
		if (idarticulo.compareTo("")!=0){
			q=q+" join c_mv_cpteinsumos i ";
			q=q+" on c.tc=i.tc and ";
			q=q+" c.idcomprobante=i.idcomprobante ";
		}
		String w="";
		if (idcomprobante.compareTo("")!=0){
			if (w.length()>0){ w=w+" and ";	}
			w=w+" c.idcomprobante like '%"+idcomprobante+"%' ";
		}
		if (idproveedor.compareTo("")!=0){
			 if (w.length()>0){ w=w+" and ";	}
			 w=w+" c.cuenta like '%"+idproveedor+"%' ";	 
		}
		
		if (nombre.compareTo("")!=0){
			if (w.length()>0){ w=w+" and ";	}
			w=w+" m.descripcion like '%"+nombre+"%'";
		...
		if (idarticulo.compareTo("")!=0){
			if (w.length()>0){ w=w+" and ";	}
			w=w+" i.idarticulo like '%"+idarticulo+"%' ";
		}
		if (w.length()>0){
			q=q+" where "+w;	
		}
		q=q+" order by c.fecha_comprobante desc ,c.cuenta,c.tc,c.idcomprobante desc";
		q=q+" ";
		return q;
	}
	
	public String getCpteStockV(String idarticulo,String idcomprobante,String idproveedor,String nombre){
		String q="";
		q=q+" select top 50 c.cuenta,m.descripcion,c.fecha,c.tc,c.idcomprobante,c.fechaentrega "; 
		q=q+" from v_mv_cpte c left outer join ";
		q=q+" ma_cuentas m on c.cuenta=m.codigo  ";
		if (idarticulo.compareTo("")!=0){
			q=q+" join v_mv_cpteinsumos i ";
			q=q+" on c.tc=i.tc and ";
			q=q+" c.idcomprobante=i.idcomprobante ";
		}
		String w="";
		...
		q=q+" order by c.fecha desc ,c.cuenta,c.tc,c.idcomprobante desc";
		q=q+" ";
		return q;
	}
