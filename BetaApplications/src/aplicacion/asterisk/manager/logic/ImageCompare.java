...
import com.sun.imageio.plugins.gif.GIFImageReader;
 ...
 public class ImageCompare {
	...
	public static void main(String[] args) {
		...
		String name1="c:\\screenshot-020625.jpg";
		String name2="c:\\screenshot-020641.jpg";
		String name3="c:\\screenshot-010713.jpg";
		ImageCompare ic = new ImageCompare(name3, name2);
		// Set the comparison parameters. 
		//   (num vertical regions, num horizontal regions, sensitivity, stabilizer)
		ic.setParameters(4, 4, 2, 2);
	...
	public ImageCompare(String file1, String file2) {
	    ...
	    File _file2 = new File(file1);
	    String formatName1 = getFormatName(_file1);
	    String formatName2 = getFormatName(_file1);
	    // e.g. gif
	    // Get image format from an input stream
	    try {
			InputStream is = new FileInputStream(_file1);
			formatName1 = getFormatName(is);
			is.close();
		...
		try {
			InputStream is = new FileInputStream(_file2);
			formatName2 = getFormatName(is);
			is.close();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		System.out.println("Format "+formatName1+"<&>"+formatName2);
		BufferedImage image1 = null;
		BufferedImage image2 = null;
		if (formatName1.compareTo("gif")==0){
			try {
				image1=ImageIO.read(_file1);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} 
		}
		if (formatName1.compareTo("jpg")==0){
			try {
				image1=this.imageToBufferedImage(this.loadJPG(file1));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} 
		...
		if (formatName2.compareTo("gif")==0){
			try {
				image2=ImageIO.read(_file2);
			} catch (IOException e) {
					// TODO Auto-generated catch block
				e.printStackTrace();
			} 
		}
		if (formatName2.compareTo("jpg")==0){
			try {
					image2=this.imageToBufferedImage(this.loadJPG(file2));
			} catch (Exception e) {
					// TODO Auto-generated catch block
				e.printStackTrace();
			} 
	...
	private String getFormatName(Object o) { 
		try { 
			...
			if (!iter.hasNext()) { // No readers found 
				return null; } // Use the first reader 
			ImageReader reader = (ImageReader)iter.next(); // Close stream 
