 ...
 * aplicacion.herramientas.java.buscadores.Pago bPago=null; public
 * void BuscarPago(JTextField ext) { if (bPago!=null){
 * bPago.close(); } bPago=new
 * aplicacion.herramientas.java.buscadores.Pago
 * (this.getConstructor().getConnectionHandler()); bPago.Buscar(ext); }
 */
public class Pago extends Generico {

	public Pago(Constructor C) {
		super(C);
	}
	...
	public void initializeLogic(JTextField tx) {
		...
		f.setWidth(150);
		logic.addFilter(f);
		logic.addRestriction(" a.cuenta like '21101%' and a.tc like 'PG' ");
