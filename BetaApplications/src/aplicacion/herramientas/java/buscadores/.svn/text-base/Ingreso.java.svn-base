 ...
 * Como utilizarlo? pegar este codigo en la aplicacion 
  private
  aplicacion.herramientas.java.buscadores.Ingreso bIngreso=null; public
  void BuscarIngreso(JTextField ext) { if (bIngreso!=null){
  bIngreso.close(); } bIngreso=new
  aplicacion.herramientas.java.buscadores.Ingreso
  (this.getConstructor()); bIngreso.Buscar(ext); }
 */
public class Ingreso extends Generico {

	public Ingreso(Constructor C) {
		super(C);
	}
	...
	public void initializeLogic(JTextField tx) {
		...
		f.setWidth(150);
		logic.addFilter(f);
		logic.addRestriction(" a.tc like 'IGR' and a.debe_haber like 'd' ");
