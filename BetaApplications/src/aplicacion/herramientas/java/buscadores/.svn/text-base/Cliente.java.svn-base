...
public void BuscarCliente(JTextField ext) {
	 if (bCliente!=null){
		 bCliente.close();
	 }
 bCliente=new aplicacion.herramientas.java.buscadores.Cliente(this.getConstructor().getConnectionHandler());
 bCliente.Buscar(ext);
} 
 */
public class Cliente extends Generico{
	...
	public void initializeLogic(JTextField tx){
		...
		logic.addRestriction("codigo like '11201%' and codigo not like '11201' AND DADA_DE_BAJA = 0 ");
		logic.init();
	}
}
