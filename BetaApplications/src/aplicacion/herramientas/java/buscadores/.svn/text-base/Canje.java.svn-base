...
/*
  ...
  private aplicacion.herramientas.java.buscadores.Canje bCanje=null; public
  void BuscarCanje(JTextField ext) { if (bCanje!=null){
  bCanje.close(); } bCanje=new
  aplicacion.herramientas.java.buscadores.Canje
  (this.getConstructor()); bCanje.Buscar(ext); }
 */
public class Canje extends Generico {

	public Canje(Constructor C) {
		super(C);
	}
	...
	public void initializeLogic(JTextField tx) {
		...
		f.setWidth(150);
		logic.addFilter(f);
		logic.addRestriction(" a.tc like 'CJE' and a.debe_haber like 'D' ");
