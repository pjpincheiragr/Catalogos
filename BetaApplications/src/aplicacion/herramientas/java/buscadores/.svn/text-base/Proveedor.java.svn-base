 ...
 * aplicacion.herramientas.java.buscadores.Proveedor bProveedor=null; public
 * void BuscarProveedor(JTextField ext) { if (bProveedor!=null){
 * bProveedor.close(); } bProveedor=new
 * aplicacion.herramientas.java.buscadores.Proveedor
 * (this.getConstructor().getConnectionHandler()); bProveedor.Buscar(ext); }
 */
public class Proveedor extends Generico {
	...
	public void initializeLogic(JTextField tx) {
		...
		logic
				.addRestriction("codigo like '21101%' and codigo not like '21101' AND DADA_DE_BAJA = 0 ");
		logic.init();
	}
}
