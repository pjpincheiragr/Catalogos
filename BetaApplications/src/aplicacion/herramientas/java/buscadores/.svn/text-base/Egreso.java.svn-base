 ...
 * Como utilizarlo? pegar este codigo en la aplicacion 
  private
  aplicacion.herramientas.java.buscadores.Egreso bEgreso=null; public
  void BuscarEgreso(JTextField ext) { if (bEgreso!=null){
  bEgreso.close(); } bEgreso=new
  aplicacion.herramientas.java.buscadores.Egreso
  (this.getConstructor().getConnectionHandler()); bEgreso.Buscar(ext); }
 */
public class Egreso extends Generico {

	public Egreso(Constructor C) {
		super(C);
	}
	...
	public void initializeLogic(JTextField tx) {
		...
		f.setWidth(150);
		logic.addFilter(f);
		logic.addRestriction(" a.tc like 'EGR' and a.debe_haber like 'd' ");
