...
import aplicacion.herramientas.java.sortableselector.constructor.*;
...
public class _Logic extends Logic {
	...
	public void clean_construction(){
	...
	public String getAutoRestrictionQuery() {
		...
		for (int i = 0; i < Filters.size(); i++) {
			if (Filters.get(i).getValor().compareTo("") != 0) {
				if (auto.length() > 0)
					auto += " and ";
				auto += Filters.get(i).getNombre() + "  like '"
						+ Filters.get(i).getValor() + "' ";
			}
		}
		return auto;
	}
	...
	private void create_table_fields() {
		...
		table.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(
				KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_DOWN, 0), "none");
		table.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(
				KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_UP, 0), "none");
		table.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(
	...
	public void focus_field(int col,JTable table){
		table.changeSelection(0, col, false,false);
		table.editCellAt(0,col);
		table.transferFocus();
	}
	...
	public void updateSizes() {
		...
		if (width > frame.getWidth()) {
			if (width <= 1000) {
				...
				frame.getJScrollPane1().setSize(width - 20,
						frame.getJScrollPane1().getHeight());
				frame.getJScrollPane().setSize(width - 20,
						frame.getJScrollPane().getHeight());
				frame.getJProgressBar().setSize(width,
						frame.getJProgressBar().getHeight());
				frame.get_toolbar_header().setSize(width,
						frame.get_toolbar_header().getHeight());
			} else {
				width = 1000;
				frame.setSize(width, frame.getHeight());
				frame.getJScrollPane1().setSize(width - 20,
	...
	public void enter(JTextField tx, int col) {
		String tex=tx.getText();
		int row = frame.getJTable().getSelectedRow();
		
		if(tex.compareTo("")==0 && row>=0)
			this.selection(tx,col);
	...
	private void customSearch() {
		if (idconector.compareTo("") != 0) {
			this.auzx = data.getConnectionHandler().getConnector(idconector).getResults(
					this.doQuerys());
		} else {
			this.auzx = data.getConnectionHandler().getDefaultConnector().getResults(
					this.doQuerys());
		}
		if (auzx != null) {
			if (auzx.length > 0) {
