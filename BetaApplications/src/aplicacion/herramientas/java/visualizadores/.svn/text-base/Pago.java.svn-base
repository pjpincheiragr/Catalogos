...
public void buscarPago(JTextField tx) {
	...
	vPago=new aplicacion.herramientas.java.visualizadores.Pago(this.getConstructor().getConnectionHandler());
	vPago.setIdproveedor(frame.get_txt_idproveedor().getText());
	int n = vPago.buscarPago(tx);
	if (n == 0) {
			aviso("no hay Pagos con ese codigo");
	}
}
*/
...
public class Pago extends Generico{
	...
	public int initializeLogic(JTextField tx) {
		...
		logic.setTitle("Buscador de Pago");
	...
	public int buscarPago(JTextField tx){
