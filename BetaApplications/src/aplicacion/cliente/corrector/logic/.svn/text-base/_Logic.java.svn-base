...
public class _Logic extends Logic {
	...
	public void _taskworkCargar(){
		...
		frame.getJProgressBar().setIndeterminate(true);
		Object[][] results =null;
		String codigo=frame.get_txt_idcliente().getText();
	...
	public void calculateDeudas(){
		...
		for (int i=0;i<frame.getJTable().getRowCount();i++){
			...
			String tc=(String)frame.getJTable().getValueAt(i, 1);
			String idcomprobante=(String)frame.getJTable().getValueAt(i, 2);
			if (tc.compareTo("FVN")==0|tc.compareTo("EGR")==0){
	...
	public void calculateCreditos(){
		...
		for (int i=0;i<frame.getJTable().getRowCount();i++){
			...
			String tc=(String)frame.getJTable().getValueAt(i, 1);
			String idcomprobante=(String)frame.getJTable().getValueAt(i, 2);
			if (tc.compareTo("CBCT")==0|tc.compareTo("NCN")==0|tc.compareTo("IGR")==0){
				if (anulado.compareTo("1")!=0){
					if (!this.hasbeenused(tc, idcomprobante)){
						this.cptes.add(new String[]{tc,idcomprobante});
	...
	private fvn vfvn=null;
	public void verFvn(String tc,String idcomprobante,Point p) {
		if (vfvn!=null){
			vfvn.close();
		}
		vfvn=new fvn(this.getConstructor());
		vfvn.initializeConstructor();
		vfvn.setTc(tc);
		vfvn.setIdcomprobante(idcomprobante);
		vfvn.setLocation(p);
		vfvn.Mostrar();
	}
	
	private aplicacion.cliente.cobranza.constructor._Constructor vCobranza=null;
	public void verCobranza(String idcobranza){
		...
		if (idcobranza.compareTo("")!=0){
				...
				vCobranza=new aplicacion.cliente.cobranza.constructor._Constructor();
				vCobranza.setParameter(_parametros.connector, this._data.getConnectionHandler());
				vCobranza.setParameter(_parametros.LookAndFeel, this.getConstructor().getLookAndFeelTheme());
				vCobranza.setParameter(aplicacion.cliente.cobranza.interfaces._Parametros.idcobranza, idcobranza);
				vCobranza.build(this.getConstructor());
				vCobranza.init();	
	...
	public void calculate(double debe,double haber,double anticipos){
		...
		double dif=saldo-saldo_utilizable;
		if (dif==0){
