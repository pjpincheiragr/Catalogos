package aplicacion.actualizacion.exportar.logic;
...
public class _Logic extends Logic {
	...
	public String getDestination(){
		...
		if (this.isLinux()){
			...
			destination="C:\\windows\\Temp\\_actualizacion";
		}
		return destination;
	}
	...
	public void generar_xml(){
		String _folder=this.getDestination();
		File folder=new File(_folder);
		if (!folder.exists()){
			System.out.println("creando directorio "+_folder);
			folder.mkdir();
		}
	        ...
	        if (results!=null){
	        	if (results.length>0){
	        		for (int i=0;i<results.length;i++){
	                	...
	                	System.out.println("actualizacion "+idactualizacion+" "+fecha+" "+idproveedor);
	        ...
	        String _file="";
	        if (this.isLinux()){
	        	_file=_folder+="/actualizacion.xml";
	        }else{
	        	_file=_folder+="\\actualizacion.xml";
	        }
	        xml._guardar(_file);
